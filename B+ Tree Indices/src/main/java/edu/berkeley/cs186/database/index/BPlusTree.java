...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        return new BPlusTreeIterator(this.root.get(key), Optional.of(key));
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> right_node  = this.root.put(key, rid);
        this.split_root(right_node);
    }

    private void split_root(Optional<Pair<DataBox, Long>> right_node) {
        if (!right_node.isPresent()) {
            return;
        }

        Long left_child = this.root.getPage().getPageNum();
        DataBox splitKey = right_node.get().getFirst();
        Long right_child = right_node.get().getSecond();
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // Use the provided updateRoot() helper method to change
        // the tree's root if the old root splits.
        if (!this.root.getClass().equals(LeafNode.class) || !(this.root.getLeftmostLeaf().getKeys().size() == 0)) {
            throw new BPlusTreeException("Tree not empty before bulk load.");
        }
        while(data.hasNext()) {
            this.split_root(this.root.bulkLoad(data, fillFactor));
        }
        return;
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        LeafNode currLeaf;
        Iterator<RecordId> currRids;
        boolean has_next;
        Optional<DataBox> floor;

        public BPlusTreeIterator(LeafNode leaf, Optional<DataBox> floor) {
            this.currLeaf = leaf;
            this.floor = floor;
            if (this.floor.isPresent()) {
                this.currRids = this.currLeaf.scanGreaterEqual(floor.get());
            } else {
                this.currRids = this.currLeaf.scanAll();
            }
            has_next = true;
            this.getSibling();
        }
        ...
        private void getSibling() {
            while (!this.currRids.hasNext()) {
                Optional<LeafNode> rightSibling = this.currLeaf.getRightSibling();
                if (rightSibling.equals(Optional.empty())) {
                    has_next = false;
                    return;
                }
                this.currLeaf = rightSibling.get();
                if (this.floor.isPresent()) {
                    this.currRids = this.currLeaf.scanGreaterEqual(floor.get());
                } else {
                    this.currRids = this.currLeaf.scanAll();
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            if (has_next && !this.currRids.hasNext()) {
                this.getSibling();
            }
            return has_next;
        }
