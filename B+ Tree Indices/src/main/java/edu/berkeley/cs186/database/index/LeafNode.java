...
class LeafNode extends BPlusNode {
    ...
    // Helper method for put and bulk load
    public void insert(DataBox key, RecordId rid) {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        List<DataBox> right_keys = new ArrayList<>(this.keys.subList(d, n));
        List<RecordId> right_rids = new ArrayList<>(this.rids.subList(d, n));
        this.keys = new ArrayList<>(this.keys.subList(0, d));
        this.rids = new ArrayList<>(this.rids.subList(0, d));

        LeafNode right_node = new LeafNode(this.metadata, this.bufferManager, right_keys, right_rids,
                                           this.rightSibling, this.treeContext);
        Long PageNum = right_node.getPage().getPageNum();
        this.rightSibling = Optional.of(PageNum);
        sync();

        return Optional.of(new Pair<>(right_keys.get(0), PageNum));
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        while(this.keys.size() < 2*d*fillFactor) {
            if (!data.hasNext()) {
                sync();
                return Optional.empty();
            }
            Pair<DataBox, RecordId> p = data.next();
            this.insert(p.getFirst(), p.getSecond());
        }
        if (!data.hasNext()) {
            sync();
            return Optional.empty();
        ...
        LeafNode right_node = new LeafNode(this.metadata, this.bufferManager,
                                           new_keys, new_rids,
                                           this.rightSibling, this.treeContext);
        Long PageNum = right_node.getPage().getPageNum();
        this.rightSibling = Optional.of(PageNum);
        sync();
        return Optional.of(new Pair<>(right_node.keys.get(0), PageNum));
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        for (int i = 0; i < n; ++i) {
            ...
            short entryId = buf.getShort();
            rids.add(new RecordId(pageId, entryId));
        }
