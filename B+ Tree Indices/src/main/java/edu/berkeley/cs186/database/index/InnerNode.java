...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        return this.getChild(this.keys.size()).get(key);
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        BPlusNode next_node = this.getChild(idx);
        Optional<Pair<DataBox, Long>> right_deets = next_node.put(key, rid);
        if (!right_deets.isPresent()) {
            return Optional.empty();
        }

        key = right_deets.get().getFirst();
        Long child = right_deets.get().getSecond();
        this.insert(key, child);
    ...
    private Optional<Pair<DataBox, Long>> split() {
        ...
        List<DataBox> right_keys = new ArrayList<>(this.keys.subList(d + 1, n));
        List<Long> right_children = new ArrayList<>(this.children.subList(d + 1, n + 1));

        DataBox split_key = this.keys.get(d);

        this.keys = new ArrayList<>(this.keys.subList(0, d));
        this.children = new ArrayList<>(this.children.subList(0, d + 1));

        InnerNode right_node = new InnerNode(this.metadata, this.bufferManager, right_keys, right_children, this.treeContext);
        Long PageNum = right_node.getPage().getPageNum();

        sync();
        return Optional.of(new Pair<>(split_key, PageNum));
    }

    private int insert(DataBox key, Long child) {
        int idx = numLessThanEqual(key, this.keys);
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        BPlusNode right_node = this.getChild(this.getChildren().size() - 1);

        while(this.keys.size() < 2*d + 1) {
            Optional<Pair<DataBox, Long>> temp = right_node.bulkLoad(data, fillFactor);
            if (!temp.isPresent()) {
                sync();
                return Optional.empty();
            }
            int idx = this.insert(temp.get().getFirst(), temp.get().getSecond());
            right_node = this.getChild(idx);
        }
