...
public class SortMergeOperator extends JoinOperator {
    ...
    private class SortMergeIterator implements Iterator<Record> {
        ...
        private Record fetchNextRecord() {
            ...
            while (!stopOnNext) {
                if (!marked) {
                    while (compare(leftRecord, rightRecord) < 0 && leftIterator.hasNext()) leftRecord = leftIterator.next();
                    while (compare(rightRecord, leftRecord) < 0 && rightIterator.hasNext()) rightRecord = rightIterator.next();
