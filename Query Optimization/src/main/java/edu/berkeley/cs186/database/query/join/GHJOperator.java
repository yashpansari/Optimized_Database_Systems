...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        int idx;
        if (left) {
            idx = getLeftColumnIndex();
        } else {
            idx = getRightColumnIndex();
        }
        for (Record record: records) {
            DataBox columnValue = record.getValue(idx);
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record buildRecord: buildRecords) {
            ...
            if (!hashTable.containsKey(buildJoinValue)) hashTable.put(buildJoinValue, new ArrayList<>());
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            if (leftPartitions[i].getNumPages() > numBuffers - 2
                    && rightPartitions[i].getNumPages() > numBuffers - 2) {
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        // SHJ has B-1 partitions, B-2 pages per buffer, 8 records per page = 160 max capacity with perfect hashing
        for (int i = 1; i <= 161; i++){
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        // GHJ breaks when there are > B-2 pages in same partition after 5 passes. Would happen if we have 8(B-1) identical values.
