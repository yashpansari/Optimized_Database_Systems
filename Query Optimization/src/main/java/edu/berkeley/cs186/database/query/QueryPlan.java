...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (!indexCols.contains(i)) {
                continue; // the selection predicate is not matched with eligible index col
            }
            QueryOperator indexScan = new IndexScanOperator(
                    transaction, table,
                    selection.column, // column name
                    selection.operator, // selection predicate
                    selection.value // DataBox object
            );
            int newCost = indexScan.estimateIOCost();
            if (newCost < minCost) {
                except = i;
                minCost = newCost;
                minOp = indexScan;
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (JoinPredicate ithJoinCondition : joinPredicates) {
            String left = ithJoinCondition.leftTable;
            String right = ithJoinCondition.rightTable;
            for (Set<String> iMinusOne : prevMap.keySet()) {
                Set<String> iKey = new HashSet<>(iMinusOne); // add i - 1 tables now and i-th later
                QueryOperator iMinusOneJoin = prevMap.get(iMinusOne);
                QueryOperator iJoin;
                if (iMinusOne.contains(left) && !iMinusOne.contains(right)) {
                    iKey.add(right);
                    iJoin = minCostJoinType(
                        iMinusOneJoin,
                        pass1Map.get(Collections.singleton(right)),
                        ithJoinCondition.leftColumn,
                        ithJoinCondition.rightColumn
                    );
                    if (!result.containsKey(iKey) || iJoin.estimateIOCost() < result.get(iKey).estimateIOCost() ) {
                        result.put(iKey, iJoin);
                    }
                } else if (iMinusOne.contains(right) && !iMinusOne.contains(left)) {
                    iKey.add(left);
                    iJoin = minCostJoinType(
                        iMinusOneJoin,
                        pass1Map.get(Collections.singleton(left)),
                        ithJoinCondition.rightColumn,
                        ithJoinCondition.leftColumn
                    );
                    if (!result.containsKey(iKey) || iJoin.estimateIOCost() < result.get(iKey).estimateIOCost() ) {
    ...
    public Iterator<Record> execute() {
        ...
        for (String table : tableNames) pass1Map.put(Collections.singleton(table), minCostSingleAccess(table));
        Map<Set<String>, QueryOperator> prevMap = pass1Map;
        while (prevMap.size() > 1) prevMap = minCostJoins(prevMap, pass1Map);
