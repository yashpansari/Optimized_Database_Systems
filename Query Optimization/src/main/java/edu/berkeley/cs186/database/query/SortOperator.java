...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        Run ans = new Run(this.transaction, this.getSchema());
        List sortedRun = new ArrayList();
        while (records.hasNext()) sortedRun.add(records.next());
        Collections.sort(sortedRun, this.comparator);
        ans.addAll(sortedRun);
        return ans;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        // Convert runs to iterators
        List<Iterator<Record>> runIterators = new ArrayList<>();
        for (Run r : runs) runIterators.add(r.iterator());

        // PQ that compares on sort column val
        PriorityQueue<Pair<Record, Integer>> upNext = new PriorityQueue<>( new RecordPairComparator() );

        // Initialize pq, result and run merge sort
        for (int i = 0; i < runs.size(); i++) if (runIterators.get(i).hasNext()) upNext.add(new Pair<>(runIterators.get(i).next(), i));
        Run ans = new Run(transaction, getSchema());
        while (!upNext.isEmpty()) {
            int i = upNext.peek().getSecond();
            ans.add(upNext.poll().getFirst());
            if (runIterators.get(i).hasNext()) upNext.add(new Pair<>(runIterators.get(i).next(), i));
        }
        return ans;
    }
    ...
    public Run sort() {
        ...
        // make runs and pass 0
        List<Run> blocks = new ArrayList<>();
        while (sourceIterator.hasNext()) blocks.add(sortRun(QueryOperator.getBlockIterator(sourceIterator, getSchema(), numBuffers)));
        ...
        return blocks.get(0); // TODO(proj3_part1): replace this!
    }
